...
export default function CourseForm({
  ...
  useEffect(() => {
    if (initialData) {
      reset({
        courseType: initialData.course_type_id,
        faculty: initialData.faculty_id,
  ...
  const submitForm = async (data) => {
    await onSubmit({
      facultyId: data.faculty || selectedFaculty,
      name: data.course,
    ...
    <form onSubmit={handleSubmit(submitForm)} className="space-y-5">
      ...
      >
        <FormSelect
          ...
          onChangeCb={(selected) => onCourseTypeChange(selected?.value || "")}
        ...
        <FormSelect
          ...
          onChangeCb={(selected) => onFacultyChange(selected?.value || "")}
        ...
        <div className="space-y-1.5 sm:space-y-2">
          ...
          <select
            ...
            className="w-full border border-border rounded-lg px-3 py-2 sm:py-2.5 bg-surface text-text text-sm sm:text-base focus:outline-none focus:ring-2 focus:ring-accent/40 focus:border-accent transition-all duration-200"
          >
