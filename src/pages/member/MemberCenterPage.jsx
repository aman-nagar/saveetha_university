...
import {
  activateMemberCenter,
  fetchMemberInactiveCenters,
} from "@/api/member/membersApi";
import * as XLSX from "xlsx";

const isCenterActive = (center) => Number(center?.is_active) === 1;
...
export default function MemberCenterPage() {
  ...
  const handleActivateCenter = async (center) => {
    if (!center?.id) return;

    setActivatingId(center.id);
    try {
      await activateMemberCenter(center.id);
      show(
        "success",
        `Center \"${center.institute_name || center.id}\" activated successfully`,
      );
      setSelectedCenter(null);
      await loadInactiveCenters(currentPage, search);
    } catch (err) {
      console.error("Failed to activate center:", err);
      show("error", err.message || "Failed to activate center");
    } finally {
  ...
  const columns = [
    ...
    },
    {
      ...
      label: "Status",
      render: (row) => {
        const active = isCenterActive(row);

        return (
          <div className="flex flex-col gap-1.5 items-start">
          <button
            type="button"
            onClick={() => handleActivateCenter(row)}
            disabled={activatingId === row.id || active}
            title={active ? "Center is already active" : "Activate center"}
            className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[11px] font-semibold border transition-colors ${
              active
                ...
                : "bg-red-500/10 text-red-600 border-red-500/20 hover:bg-emerald-500/10 hover:text-emerald-600 hover:border-emerald-500/20"
            }`}
          >
            {activatingId === row.id ? (
      ...
      <Modal
        ...
        footer={
          <div className="flex justify-between items-center w-full">
            <div className="flex items-center gap-2">
              <Button
                onClick={() => handleActivateCenter(selectedCenter)}
                loading={activatingId === selectedCenter?.id}
                disabled={isCenterActive(selectedCenter)}
              ...
              <div className="flex-1 text-center sm:text-left space-y-1">
                <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2">
                  <span className={`px-2.5 py-0.5 rounded-full text-xs font-semibold border ${
                    isCenterActive(selectedCenter)
                      ? "bg-emerald-500/10 text-emerald-600 border-emerald-500/20"
                      : "bg-red-500/10 text-red-600 border-red-500/20"
                  }`}>
                    Status: {isCenterActive(selectedCenter) ? "Active" : "Inactive"}
                  </span>
                  <span
